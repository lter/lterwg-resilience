...
grass.fert <- dat2 %>%
  ...
  dplyr::summarise(anpp_stability = sd(anpp_g_m2 / mean(anpp_g_m2)),
                   length = n()) %>%
  subset(length > 2) %>%
  dplyr::select(site, treatment, type2, anpp_stability, fertilized, ppt_max_event,  ppt_mean_event,days_half_ppt,daily_ppt_d,n_wet_days,avg_dryspell_length,ppt_95th_percentile_size,MAP,cv_ppt_intra,cv_ppt_inter,yearly_ppt_d,seasonality_index )
colnames(dat2)
...
pdp_plots <- pdp_data %>%
  ...
  purrr::map(function(df) {
    ggplot(df, aes(x = x, y = estimate)) +
      ...
      labs(
        x     = unique(df$label),
        y     = "Treatment effect on ANPP stability",
        title = NULL
      ) +
      theme(
...
grass.corn <- dat2 %>%
  ...
  dplyr::select(site, treatment, type2, anpp_g_m2, fertilized, ppt_max_event,  ppt_mean_event,days_half_ppt,daily_ppt_d,n_wet_days,avg_dryspell_length,ppt_95th_percentile_size,MAP,cv_ppt_intra,cv_ppt_inter,yearly_ppt_d,seasonality_index ) %>%
  group_by(site, treatment, type2, fertilized, ppt_max_event,  ppt_mean_event,days_half_ppt,daily_ppt_d,n_wet_days,avg_dryspell_length,ppt_95th_percentile_size,MAP,cv_ppt_intra,cv_ppt_inter,yearly_ppt_d,seasonality_index ) %>%
  dplyr::summarise(anpp_stability = sd(anpp_g_m2 / mean(anpp_g_m2, na.rm = TRUE)),
                   length = n()) %>%
  subset(length > 2) %>%
  dplyr::select(site, treatment, type2, anpp_stability, fertilized, ppt_max_event,  ppt_mean_event,days_half_ppt,daily_ppt_d,n_wet_days,avg_dryspell_length,ppt_95th_percentile_size,MAP,cv_ppt_intra,cv_ppt_inter,yearly_ppt_d,seasonality_index )%>%
  subset(site != "WICST_ARS" & site != "MO_Knox1")#this had some problem
